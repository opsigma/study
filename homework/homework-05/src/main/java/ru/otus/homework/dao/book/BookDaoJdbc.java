...
public class BookDaoJdbc implements BookDao {
    ...
    private MapSqlParameterSource getParameterMap(Book book) {
        ...
        map.addValue("autor_id", optionalBook.map(Book::getAuthor).map(Author::getId).orElse(null));
        map.addValue("genere_id", optionalBook.map(Book::getGenre).map(Genre::getId).orElse(null));
        return map;
    }
